...
void bitInverseChunk(int index, int layer, TrieNode* ports)
{
	Chunk* chkptr = ltb[index].chk;
	ushort ** base = chkptr->base;
	ushort GROUP_ = chkptr->groupWidth;
	int depth = layer==2? 6: 8;
	ushort CLUSTER_ = depth - BIT -  GROUP_;

	uint range = 1 << (CLUSTER_ + BIT);
	int group, cluster, bit;
	int i, start;
	for (i = 0, group = 0; group < (1 << GROUP_); i += range, group++)
	{
		ushort *bitmask = (ushort *)malloc(sizeof(ushort) * (range));
		memset(bitmask, 0, sizeof(ushort) * range);
		bitmask[0] = 1;
		int cnt = 1;
		TrieNode *port_temp = (TrieNode *)malloc(sizeof(TrieNode) * (range));

		uint inverse_sequence = bit_inverse(ports + group * range, range);
		for (start = 0; start <  range; start++)
			port_temp[start^inverse_sequence] = ports[start + i];

		TrieNode previous = port_temp[0];
		for (start = 1; start < range; start++){
			if (previous.iptr != port_temp[start].iptr){
				cnt++;
				bitmask[start] = 1;
				previous.iptr = port_temp[start].iptr;
			}
		}
		...
		for (cluster = 0; cluster < (1 << CLUSTER_); cluster++)
		{
			num8 = 0;
			curOne = 0;
			for (bit = (cluster << BIT); bit < ((cluster + 1) << BIT); bit++)
			{
				num8 <<= 1;
				if (bitmask[bit] == 1)
				{
...
void makeltblow3(TrieNode ** v8ptr, ushort current)
{
	...
	memset(bitmask, 0, sizeof(ushort) * (1 << 8));
	...
	else
	{
		...
		bitInverseChunk(current, 3, ports);
...
void makeltblow2(TrieNode ** v8ptr, ushort current) 
{
	...
	memset(bitmask, 0, sizeof(ushort) * (1 << LAYER2));
	...
	else
	{
		...
		ltb[current].chk = chkptr;
		bitInverseChunk(current, 2, ports);
	}
	free(bitmask);
	free(ports);
}
...
void modifyltb_spa2bit(TrieNode ** v8ptr, ushort current, ushort portfix, int cnt, Special * special, TrieNode * lookup, int layer_n)
{
	...
	memset(bitmask, 0, sizeof(ushort) * (1 << layer));
...
uint bit_inverse(TrieNode* ports, uint range){
	uint sequenceBest = 0;
	TrieNode* newPorts = (TrieNode *) malloc(sizeof(TrieNode) * (range));
	uint sumMin = range+1;
	for (uint sequence = 0; sequence < range; sequence++)
	{
		uint sum = 0;
		TrieNode previous;
		previous.iptr = 0;
		for (uint i = 0; i < range; i++)
			newPorts[i^sequence] = ports[i];
		for (uint i = 0; i < range; i++){
			if (previous.iptr != newPorts[i].iptr){
				sum++;
				previous.iptr = newPorts[i].iptr;
			}
		}
		if (sum < sumMin){
			sumMin = sum;
	...
	free(newPorts);
	//printf("Best sequence: %u, \t Minimum sum: %u", sequenceBest, sumMin);
	return sequenceBest;
}
