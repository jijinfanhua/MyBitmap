 ...
 */
struct poptrie * poptrie_init(struct poptrie *poptrie, int sz1, int sz0) //19  22
{
    int ret;
    int i;
    //unsigned long long space_size1 = 0;
    if ( NULL == poptrie ) {
        /* Allocate new one */
	...
	//space_size1 += sizeof(struct poptrie);
    ...
    poptrie->nodes = malloc(sizeof(poptrie_node_t) * (1 << sz1)); // initialize the size of nodes that should be allocated
    for(i = 0; i<(1<<sz1); i++)
    {
	poptrie->nodes[i].leafvec = 0;
	poptrie->nodes[i].vector = 0;
	poptrie->nodes[i].base0 = 0;
	poptrie->nodes[i].base1 = 0;
    }
    //space_size1 += sizeof(poptrie_node_t) * (1 << sz1);
    if ( NULL == poptrie->nodes ) {// initialize failed
        poptrie_release(poptrie);
        return NULL;
    }
    poptrie->leaves = malloc(sizeof(poptrie_leaf_t) * (1 << sz0));//initialize the size of leaves that should be allocated
    for(i = 0; i<(1<<sz0); i++)
    {
	poptrie->leaves[i] = 0;
    }
    //space_size1 += sizeof(poptrie_leaf_t) * (1 << sz0);
    if ( NULL == poptrie->leaves ) {// initialize failed
        poptrie_release(poptrie);
        return NULL;
    }
    ...
    //space_size1 += sizeof(struct buddy);
    ...
    //space_size1 += sizeof(u32) << POPTRIE_S;

    if ( NULL == poptrie->dir ) {
        poptrie_release(poptrie);
        return NULL;
    }
    for ( i = 0; i < (1 << POPTRIE_S); i++ ) {// here not plus
    ...
    poptrie->fib.entries = malloc(sizeof(struct poptrie_fib_entry) * POPTRIE_INIT_FIB_SIZE);
    ...
    //space_size1 += sizeof(struct poptrie_fib_entry) * POPTRIE_INIT_FIB_SIZE;
    ...
    //printf("poptrie initialize: %llu bytes!\n", space_size1);
