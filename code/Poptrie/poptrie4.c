...
/*
 * Lookup a route by the specified address \\ void *
 */
void * poptrie_lookup(struct poptrie *poptrie, u32 addr)
{
    ...
    if ( poptrie->dir[idx] & ((u32)1 << 31) ) {
        return (poptrie->fib.entries[poptrie->dir[idx] & (((u32)1 << 31) - 1)].entry);//modify
    } else {
    ...
    for ( ;; ) {
        ...
        if ( VEC_BT(poptrie->nodes[inode].vector, idx) ) {
            ...
            return (poptrie->fib.entries[poptrie->leaves[base + idx - 1]].entry);//modify
        }
    }
