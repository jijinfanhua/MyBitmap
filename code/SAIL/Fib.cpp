...
void CFib::sailTableInit()
{
	...
	if (!levelTable16 || !levelTable24 || !levelTable32) {
		...
		//fgetc(stdin);
		//system("pause");  not portability and very expensive
		_exit(-1);// in windows is exit(-1)
	}
...
unsigned int CFib::BuildFibFromFile(string sFileName)
{
	...
	while (!fin.eof()) {			// Add by Qiaobin Fu 2014-4-22
		...
		/***************zhang chuwen modify 2017.7.25**************/
		//printf("%d\n", iNextHop);
		//iNextHop = findMap(iNextHop);
...
unsigned int CFib::sailLookup(unsigned int ip)
{
	...
	if (levelTable16->element[offset16] < 0)	
		LPMPort = (0 - levelTable16->element[offset16]);
...
unsigned int * CFib::TrafficRead(char *traffic_file)
{
	...
	{
		//printf("legends never die...\n");
		fscanf(fp, "%u", &traffic[count]);
		//printf("%u\n", traffic[count]);
		count++;
		//getchar();
	}
	printf("hahahhahsdfsdfa\n");
	if (count <TRACE_READ)
	{
		printf("not enough\n",count);
	}
	//printf("size1 : %llu\n", currentChunkNum24 * sizeof(struct levelTable24*) + currentChunkNum32 * sizeof(struct levelTable32*));
	return traffic;
}
...
unsigned int  CFib::findMap(unsigned int port) 
{
	int i = 1;
